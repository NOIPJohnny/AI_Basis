...
class Net(nn.Module):
    def __init__(self, embedding_size=64, hidden_size=128, num_classes=2):
        ...
        self.lstm1 = LSTM(input_size=embedding_size, hidden_size=hidden_size)
    ...
    def forward(self, x):
        x = self.embedding(x)
        x = x.permute(1, 0, 2)
        outputs1, (h_t1, c_t1) = self.lstm1(x)
        outputs2, (h_t2, c_t2) = self.lstm2(outputs1)
        x = torch.mean(outputs2, dim=0)
        x = self.fc1(x)
        x = self.relu(x)
        x = self.dropout(x)
