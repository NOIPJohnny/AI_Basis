...
class ImdbNet(Module):
    ...
    def forward(self, x):
        ...
        prev_h = torch.zeros(num_of_layers, batch_size_in_forward, hidden_size).to(device)
        prev_c = torch.zeros(num_of_layers, batch_size_in_forward, hidden_size).to(device) 
        x = self.embedding(x)
