...
double temperature_schedule_move(int time){
    ...
    static double start_temp_log = n/log_n*0.7;
    return exp(start_temp_log - double(time) / (n << 5));
}
...
int main(){
    ...
    sa.search(value_estimator_move, temperature_schedule_move, n << 4, max_conflicts, 1e-30);
